...
class Model
{
	...
	/******************************************************************/
	/**
	 * Include all relationships ( modify by wanghm 2016-11-25 )
	 */
	/******************************************************************/
	private function include_relationships(array $options=array()) {
	
		// init result array
		$includes = array_keys($this->__relationships);
	
		// add result to $options
		if (isset($options['include'])) {
			array_merge($options['include'], $includes);
		} else {
	...
	{
		$options = $this->include_relationships($options);
